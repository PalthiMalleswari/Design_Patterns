...
class FileDataSource(DataSource):
    ...
    def read_data(self):
        ...
        print(f"Reading from {self.file_name} File")
...
class Application:
    ...
    def main():
        ...
        file = FileDataSource("python-ex.py")

        file.write_data("First Line")
        file.read_data()

        encrypt_file = EncryptionDecorator(file)
        encrypt_file.write_data("second Line")
        encrypt_file.read_data()

        compres_encrypt_file = CompressionDecorator(encrypt_file)
        compres_encrypt_file.write_data("Writing a Third Line")
        compres_encrypt_file.read_data()

        compres_file = CompressionDecorator(file)
        compres_file.write_data("Writing a Only Compressed Data")
        compres_file.read_data()
